...
4.  Below are histograms with three different binwidth values. I think a binwidth of 0.10 reveals the most interesting pattern. Diamonds tend to be cut to achieve a full carat weight to maximize profits.
...
1.  `manufacturer`, `model`, `trans`, `drv`, `fl`, and `class` are all categorical variables. `displ`, `year`, `cyl`, `cty`, and `hwy` are all numerical variables. You can run `glimpse(mpg)` or `?mpg` to see a list of the variables.
...
5.  Adelies tend to have higher bill depth while Gentoo have longer bills and Chinstrap have deeper and longer bills.
    ...
    ggplot(
      ...
      geom_point()
    ```
This is an example of Simpson's paradox. It seems like there is a slight negative correlation between the two variables but the trend reverses when the data is split per species. 
